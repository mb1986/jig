...
**Priority: high.** This is arguably the most valuable post-v1 feature
and was explicitly called out as essential for v2.

Static completion (v1, via `clap_complete`) handles `jig`'s own flags
(`--dry-run`, `--config`, etc.) and tells the shell that the next
positional is "a command name." But it cannot know *which* command
names, aliases, or profiles exist — those depend on the local
`jig.kdl`.

Dynamic completion would complete:

- Command names and aliases for the first positional after `jig`
  (or after `jig`'s own flags).
- Profile names for the second positional, scoped to the chosen command.
- Optionally: nothing further, since pass-through args are by definition
  arguments to a different program.

### Possible approaches

1. **`clap_complete` dynamic API** (`unstable-dynamic` feature). The
   completion script calls back into `jig` at completion time, which
   reads the local `jig.kdl` and returns candidates. As of v1
   implementation this API is unstable; revisit when it stabilizes.

2. **Hand-rolled completion script** that calls `jig --list-commands`
   or `jig --list-profiles <command>` (new flags emitting one
   candidate per line). The shell-side script is small and stable
   even while the candidate-generation side evolves.

3. **Hybrid:** static script generated by `clap_complete`, augmented
   with a small shell function that handles the dynamic candidates.

Approach 2 is probably the most robust. The interface
(`jig --list-commands` returning newline-separated names) is trivial to
implement, easy to test, and shell-agnostic.

### Ergonomic priorities

Order of importance for the user experience:

1. **Zsh** — primary target.
2. **Bash** — broad compatibility.
3. **Fish** — popular among Rust users.
4. **Other shells** as bonus.
...
Machine-readable form of `jig --list` for use by scripts, completion
systems, and the dynamic completion feature above. Probably implemented
together with dynamic completion since they share infrastructure.
